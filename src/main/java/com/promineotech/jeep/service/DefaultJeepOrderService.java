...
public class DefaultJeepOrderService implements JeepOrderService {
  ...
  protected Jeep getModel(OrderRequest orderRequest) {
    ...
    return jeepOrderDao.fetchModel(
        orderRequest.getModel(), orderRequest.getTrim(), orderRequest.getDoors())
        .orElseThrow(() -> new NoSuchElementException(
            "Model with ID=" + orderRequest.getModel() 
            + ", trim =" + orderRequest.getTrim()
            + ", doors=" + orderRequest.getDoors() 
            + " was not found"));
    // @formatter:on
  } // end getModel
